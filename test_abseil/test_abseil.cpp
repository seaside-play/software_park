...
ABSL_FLAG(std::string, log_dir, "C:\\Users\\wisechip\\Desktop\\temp",
    "directory to write log files");
ABSL_FLAG(std::string, program_name, "test_abseil",
    "program name to use in log files");

void InitializeLogging() {
    std::string log_filename = absl::StrCat(absl::GetFlag(FLAGS_program_name), ".",
        absl::FormatTime("%Y%m%d", absl::Now(), absl::LocalTimeZone()),
        ".log");
    std::string log_filepath = absl::StrCat(absl::GetFlag(FLAGS_log_dir), "\\", log_filename);
...
namespace wisechip
{
    namespace base
    {
        ...
        public:
            ...
            ~LogInitializer() {
                absl::RemoveLogSink(&line_printer_log_sink_);
            }
        private:
...
{
    absl::InitializeLog();
    //wisechip::base::LogInitializer log("C:\\Users\\wisechip\\Desktop\\temp\\1.txt");
